...
Test(T, push_insertion, .init = setup, .fini = teardown) {
  ...
  JSON_Hashmap_Entry item ={.key=key0,.type=Item_string,.value=strdup(value0)};
  map->push(map ,item);
  cr_assert_eq(map->length(map), 1, "Wrong length");
  char key1[10] = "Euro", value1[10] = "€";
  JSON_Hashmap_Entry item2 ={.key=key1,.type=Item_string,.value=strdup(value1)};
  map->push(map ,item2);
...
Test(T, get_case_sensitive, .init = setup, .fini = teardown) {
  ...
  JSON_Hashmap_Entry item ={.key=key,.type=Item_string,.value=strdup(value)};
  map->push(map, item);
...
Test(T, push_nested_hashmap, .init = setup, .fini = teardown) {
  ...
  JSON_Hashmap_Entry item ={.key="DOLLAR",.type=Item_string,.value=strdup("$")};
  map2->push(map2,item);
...
Test(T, push_replacement, .init = setup, .fini = teardown) {
  JSON_Hashmap_Entry item = {.key="Symbols",.type=Item_string,.value=strdup("$")};
  map->push(map, item);
  JSON_Hashmap_Entry item2 = {.key="Symbols",.type=Item_string,.value=strdup("€")};
  map->push(map, item2);
...
Test(T, entries_simple, .init=setup, .fini = teardown) {
  JSON_Hashmap_Entry item ={.key="DOLLAR",.type=Item_string,.value=strdup("$")};
  map->push(map, item);
  JSON_Hashmap_Entry item2 = {.key="EURO",.type=Item_string,.value=strdup("€")};
  map->push(map, item2);
  JSON_Hashmap_Entry**entries = map->entries(map);
...
Test(T, to_json_double, .fini = teardown) {
  ...
  *price = 2.5;
  *amount = 2;
  JSON_Hashmap_Entry item ={.key="price",.type=Item_double,.value=price};
  map->push(map ,item);
  JSON_Hashmap_Entry item2 ={.key="amount",.type=Item_double,.value=amount};
  map->push(map ,item2);
  char *json = map->to_json(map);
  cr_assert_eq(strcmp(json,res),0, "Should be equal");
}
...
Test(T, to_json_int, .fini = teardown) {
  ...
  *amount = 2;
  ...
  map->push(map ,item);
  JSON_Hashmap_Entry item2 ={.key="amount",.type=Item_int,.value=amount};
  map->push(map ,item2);
  char *json = map->to_json(map);
  cr_assert_eq(strcmp(json,res),0, "Should be equal");
}
...
Test(T, to_json_boolean, .fini = teardown) {
  ...
  *amount = true;
  ...
  map->push(map ,item);
  JSON_Hashmap_Entry item2 ={.key="amount",.type=Item_bool,.value=amount};
  map->push(map ,item2);
  char *json = map->to_json(map);
  cr_assert_eq(strcmp(json,res),0, "Should be equal");
}
...
Test(T, to_json_null, .fini = teardown) {
  ...
  map->push(map ,item);
  JSON_Hashmap_Entry item2 ={.key="amount",.type=Item_null,.value=amount};
  map->push(map ,item2);
  char *json = map->to_json(map);
  cr_assert_eq(strcmp(json,res),0, "Should be equal");
}
...
Test(T, to_json_hashmap, .fini = teardown) {
  ...
  map->push(map ,item3);
  JSON_Hashmap_Entry item4 ={.key="amount",.type=Item_map,.value=amount};
  map->push(map ,item4);
  char *json = map->to_json(map);
  cr_assert_eq(strcmp(json,res),0, "Should be equal");
}
...
Test(T, to_json_array, .fini = teardown) {
  ...
  array->push(array,(Item){.type=Item_string,.value=strdup("£")});
  map->push(map,(JSON_Hashmap_Entry){.key="price",.type=Item_array,.value=array});
  char *json = map->to_json(map);
  cr_assert_eq(strcmp(json,res),0, "Should be equal");
}
